...
def main():
    files = sorted(Path(p) for p in glob.glob("saved_*/parent_rvecs.txt"))
    if not files:
        raise SystemExit("No files matched saved_*/parent_rvecs.txt")
    ...
    cx, cy = -3.0, 0.0
    sigma_R = 1.0 / 2.0
    for r, lw, label in [(1.0 * sigma_R, 1.5, r"$\sigma$"), (2.0 * sigma_R, 1.5, r"$2\sigma$")]:
    ...
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    ax.set_title("Parent rvecs: scatter with σ and 2σ (μ = (-3, 0), σ = 1)")
    ...
    out = Path("parent_rvecs_scatter.png")
    fig.tight_layout()
