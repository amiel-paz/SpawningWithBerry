...
for j in seed_idx:
    ...
    temp_bundle = Bundle(ndim,model)
    temp_bundle.add_trajectory(r, p, child_state)
    # backprop with the exact forward step h
    for _ in range(int(saved_steps[j])):
        solverk45_and_step_classical(temp_bundle, -h, verbose=False)
    TBF = temp_bundle.trajectorylist[0]
    r = TBF.rvec
