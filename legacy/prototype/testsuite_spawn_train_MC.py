...
for samp_idx in range(1,23):
    ...
    for step in range(nstep):
        if step % check_every == 0:
            ...
            save_state_density_frame_fixed(f"saved_{samp_idx}/frames_adiabatic", step, X, Y, rho0, state_index=0)
            save_state_density_frame_fixed(f"saved_{samp_idx}/frames_adiabatic", step, X, Y, rho1, state_index=1)
            np.savetxt(f"saved_{samp_idx}/frame_{step}_GS_pop.txt",rho0)
            np.savetxt(f"saved_{samp_idx}/frame_{step}_XS_pop.txt",rho1)
        advance_one_step(bundle, h, nslice=20, move_nuclei=True, renormalize=True)
    
    os.makedirs(f"./saved_{samp_idx}",exist_ok=True)
    np.savetxt(f"saved_{samp_idx}/pops_xs.txt",np.asarray(pops_xs))
    np.savetxt(f"saved_{samp_idx}/pops_gs.txt",np.asarray(pops_gs))
    np.savetxt(f"saved_{samp_idx}/pops_steps.txt",np.asarray(pops_steps))
    np.savetxt(f"saved_{samp_idx}/pops_times.txt",np.asarray(pops_steps)*h)
