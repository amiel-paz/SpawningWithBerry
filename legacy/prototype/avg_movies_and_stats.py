...
EXPECTED_STEPS = list(range(0, 598 + 1, 20))  # strict: 0..4980 by 20
...
def try_load_exact(root: Path):
    ...
    # shape sanity
    n = len(out["times"])
    for k in ("pop_xs","pop_gs","px_xs","py_xs"):
        if out[k].ndim != 1 or len(out[k]) != n:
            return None
    return out

def plot_with_ci_and_exact(times, mean, lo, hi,
                           title, outpng,
                           exact=None, pop_clip=False,
                           momentum_pad=0.0,
                           ylabel="population"):
    import matplotlib.pyplot as plt
    fig, ax = plt.subplots(figsize=(6,4), dpi=140)
    ax.fill_between(times, lo, hi, alpha=0.25, linewidth=0)
    ax.plot(times, mean, linewidth=1.75)

    if exact is not None:
        t_e, y_e = exact
        ax.plot(t_e, y_e, "k", linewidth=1.8, label="exact")
        ax.set_xlim(float(np.min(t_e)), float(np.max(t_e)))
        ymin = float(np.min(y_e))
        ymax = float(np.max(y_e))
        if pop_clip:
            ymin = max(0.0, ymin)
            ymax = min(1.0, ymax)
        else:
            ...
            ymax += momentum_pad
        if ymin == ymax:
            ymin -= 1e-12; ymax += 1e-12
        ax.set_ylim(ymin, ymax)

    ax.set_xlabel("time"); ax.set_ylabel(ylabel)
    ax.set_title(title)
    if exact is not None: ax.legend(frameon=False)
...
def main():
    ...
    for k, dirs in groups.items():
        ...
        # Populations (overlay exact if present)
        plot_with_ci_and_exact(
            times, xs_mean, xs_lo, xs_hi,
            f"Excited population (k={k}, mean ± 95% CI)",
            stats_out / "pop_excited_ci.png",
            exact=None if exact is None else (exact["times"], exact["pop_xs"]),
            pop_clip=True, ylabel="population"
        )
        plot_with_ci_and_exact(
            times, gs_mean, gs_lo, gs_hi,
            f"Ground population (k={k}, mean ± 95% CI)",
            stats_out / "pop_ground_ci.png",
            exact=None if exact is None else (exact["times"], exact["pop_gs"]),
            pop_clip=True, ylabel="population"
        )
        ...
        # Plots
        # Momenta (overlay exact; ±5 padding)
        plot_with_ci_and_exact(
            times, xs_px_mean, xs_px_lo, xs_px_hi,
            f"Excited πx (k={k})",
            stats_out / "px_excited_ci.png",
            exact=None if exact is None else (exact["times"], exact["px_xs"]),
            pop_clip=False, momentum_pad=5.0, ylabel="πx (mechanical)"
        )
        plot_with_ci_and_exact(
            times, xs_py_mean, xs_py_lo, xs_py_hi,
            f"Excited πy (k={k})",
            stats_out / "py_excited_ci.png",
            exact=None if exact is None else (exact["times"], exact["py_xs"]),
            pop_clip=False, momentum_pad=5.0, ylabel="πy (mechanical)"
        )
